...
Log.prettyStackTrace = function(obj){
  var stack;
  if( ! obj.e ) stack=printStackTrace();
  else stack=printStackTrace({e: obj.e});

  if( obj.isFatal ) lvl=Log.e.FATAL;
  else lvl=Log.e.ERROR;

  if( obj.msg ) Log.custom(lvl, obj.msg);

  stack.forEach(function(v,k){
    Log.custom(lvl, v);
  });

  if( obj.isFatal ) process.exit(1);
}
